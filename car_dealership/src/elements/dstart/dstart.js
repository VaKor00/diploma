...
import startdealers from "../../startdealers.json";
...
function DStart() {
   ...
   const MyMarker = React.memo(function MyMarker({ feature }) {
    ...
    const handleCityChange = (event) => {
        const cityID = parseInt(event.target.value, 10); // Преобразуем ID в число
    ...
    const setCityCoordinates = (cityID) => {
         // Получение координат города из API Яндекс.Карт
         fetch(`https://geocode-maps.yandex.ru/v1/?apikey=${apiKey}&geocode=${city[cityID].city}&format=json`)
         .then((res) => res.json())
         .then((data) => {
